...
const createChart = async () => {
  ...
  const currentYearMap = new Map(
    sortedData.value.map((d) => [formatMonth(d.month), d.deliveryPrice])
  )
  const nextYearMap = new Map(
    sortedNextYearData.value.map((d) => [formatMonth(d.month), d.deliveryPrice])
  )

  const x = d3.scaleBand().domain(months).range([0, width]).padding(0.2)

  const maxDeliveryPrice = Math.max(
    d3.max(sortedData.value, (d) => d.deliveryPrice) || 1000,
    hasNextYearData
      ? d3.max(sortedNextYearData.value, (d) => d.deliveryPrice) || 1000
      : 0
  )

  const y = d3
    .scaleLinear()
    .domain([0, maxDeliveryPrice])
    .nice()
    .range([height, 0])
  ...
  const currentYearDataForLine = months.map((month) => ({
    month,
    deliveryPrice: currentYearMap.get(month) || 0
  }))

  const nextYearDataForLine = months.map((month) => ({
    month,
    deliveryPrice: nextYearMap.get(month) || 0
  }))

  const line = d3
    .line<{ month: string; deliveryPrice: number }>()
    .x((d) => (x(d.month) || 0) + x.bandwidth() / 2)
    .y((d) => y(d.deliveryPrice))
    .curve(d3.curveMonotoneX)

  const area = d3
    .area<{ month: string; deliveryPrice: number }>()
    .x((d) => (x(d.month) || 0) + x.bandwidth() / 2)
    .y0(height)
    ...
    svg
      ...
      .on('mouseover', function (event, d) {
        ...
        tooltip
          .style('opacity', 1)
          .html(
            `<strong>${nextYear}</strong><br>Frais de livraison: ${formatCurrency(d.deliveryPrice)}`
          )
          .style('left', event.pageX + 10 + 'px')
          .style('top', event.pageY - 20 + 'px')
      })
