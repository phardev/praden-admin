...
const createChart = async () => {
  ...
  if (hasPreviousYearData) {
    ...
    const previousYearDataAligned = sortedData.value.map((d) => {
      ...
      return {
        month: monthNum,
        deliveryPrice: prevData?.deliveryPrice || 0
      }
    })
    ...
    svg
      ...
      .on('mouseover', function (event, d) {
        ...
        tooltip
          .style('opacity', 1)
          .html(
            `<strong>${previousYear}:</strong> ${formatCurrency(d.deliveryPrice)}`
          )
  ...
  svg
    ...
    .on('mouseover', function (event, d) {
      ...
      tooltip
        .style('opacity', 1)
        .html(
          `<strong>${currentYear}:</strong> ${formatCurrency(d.deliveryPrice)}`
        )
        .style('left', event.pageX + 10 + 'px')
        .style('top', event.pageY - 20 + 'px')
    })
