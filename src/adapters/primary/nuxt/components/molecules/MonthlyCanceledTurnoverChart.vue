...
const createChart = async () => {
  ...
  const currentYearMap = new Map(
    sortedData.value.map((d) => [formatMonth(d.month), d.canceledTurnover])
  )
  const nextYearMap = new Map(
    sortedNextYearData.value.map((d) => [
      formatMonth(d.month),
      d.canceledTurnover
    ])
  )
  ...
  const maxCanceledTurnover = Math.max(
    d3.max(sortedData.value, (d) => d.canceledTurnover) || 1000,
    hasNextYearData
      ? d3.max(sortedNextYearData.value, (d) => d.canceledTurnover) || 1000
      : 0
  )

  const y = d3
    .scaleLinear()
    .domain([0, maxCanceledTurnover])
    .nice()
    .range([height, 0])
  ...
  const currentYearDataForLine = months.map((month) => ({
    month,
    canceledTurnover: currentYearMap.get(month) || 0
  }))

  const nextYearDataForLine = months.map((month) => ({
    month,
    canceledTurnover: nextYearMap.get(month) || 0
  }))

  const line = d3
    .line<{ month: string; canceledTurnover: number }>()
    .x((d) => (x(d.month) || 0) + x.bandwidth() / 2)
    .y((d) => y(d.canceledTurnover))
    .curve(d3.curveMonotoneX)

  const area = d3
    .area<{ month: string; canceledTurnover: number }>()
    .x((d) => (x(d.month) || 0) + x.bandwidth() / 2)
    .y0(height)
    .y1((d) => y(d.canceledTurnover))
    .curve(d3.curveMonotoneX)
  ...
  svg
    ...
    .attr('cy', (d) => y(d.canceledTurnover))
    .attr('r', 5)
    ...
    .on('mouseover', function (event, d) {
      ...
      tooltip
        .style('opacity', 1)
        .html(
          `<strong>${currentYear}</strong><br>CA Annulé: ${formatCurrency(d.canceledTurnover)}`
        )
    ...
    svg
      ...
      .on('mouseover', function (event, d) {
        ...
        tooltip
          .style('opacity', 1)
          .html(
            `<strong>${nextYear}</strong><br>CA Annulé: ${formatCurrency(d.canceledTurnover)}`
          )
          .style('left', event.pageX + 10 + 'px')
          .style('top', event.pageY - 20 + 'px')
      })
...
watch(
  ...
  (newData) => {
    if ((newData as MonthlySales[][])?.length > 0) {
      createChart()
    }
  },
  { deep: true }
