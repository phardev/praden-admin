...
UModal(:model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)")
  UCard
    template(#header)
      h3.text-lg.font-semibold {{ $t('shopManagement.pharmacistSelection.searchProducts') }}

    template(#default)
      .space-y-4
        UInput(
          v-model="searchQuery"
          :placeholder="$t('shopManagement.pharmacistSelection.searchProducts')"
        ...
        div(v-else-if="searchVM.searchResults.length === 0")
          .text-center.py-8.text-gray-500
            icon.mb-2(name="i-heroicons-magnifying-glass" class="text-4xl")
            p {{ $t('shopManagement.pharmacistSelection.noResults') }}

        div(v-else)
          h4.text-md.font-semibold.mb-3 {{ $t('shopManagement.pharmacistSelection.searchResults') }}
          .space-y-2.max-h-96.overflow-y-auto
            .product-item.p-4.bg-white.border.rounded.flex.items-center.justify-between.transition-all(
              ...
              span.text-sm.text-gray-500(v-else) {{ $t('shopManagement.pharmacistSelection.alreadySelected') }}
</template>
