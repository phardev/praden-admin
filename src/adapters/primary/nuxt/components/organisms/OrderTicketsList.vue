...
div.text-center.py-8(v-else-if="!vm.isLoading && vm.tickets.length === 0")
  .space-y-2
    ...
    p.text-sm.text-muted {{ $t('support.noTicketsForOrder') }}
...
import { getOrderTicketsVM } from '@adapters/primary/view-models/orders/get-order-tickets/getOrderTicketsVM'
...
const vm = computed(() => getOrderTicketsVM(props.orderUuid))
